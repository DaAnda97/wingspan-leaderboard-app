...
export const initDBs = () => {
    initScoresDB()
        ...
        .catch((error: Error) =>
            console.log(
                `Scores Database initialisation failed with error: ${error}`
            )
        );
    ...
    initPlayersDB()
        ...
        .catch((error: Error) =>
            console.log(
                `Players Database initialisation failed with error: ${error}`
            )
        );
};
...
export const insertScoring = (scoring: Scoring) => {
    return new Promise((resolve, reject) => {
        db.transaction((tx) => {
            tx.executeSql(
                'INSERT INTO scores (scoringSheetId, playerId, roundPoints, bonusPoints, eggPoints, foodPoints, nectarPoints, birdPoints, cardPoints, totalScore) VALUES  (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                [
...
export const drop = () => {
    return new Promise((resolve, reject) => {
        db.transaction((tx) => {
            tx.executeSql(
                'DROP TABLE scores; DROP TABLE players',
                [],
                (_, result) => {
                    resolve(result);
                },
